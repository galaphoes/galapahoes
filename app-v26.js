...
async function loadData(){
 ...
 async function tryFeed(label,url,apply,render){
  ...
  try{
   ...
   if(label==="Activities"){
    console.table(state.activities.map(a=>({
     id:a.id,
     activity:a.name,
     date:a.date
    })));
   ...
   if(label==="Signups"){
    console.table(state.signups.map(s=>({
 ...
 await Promise.all([
  ...
  tryFeed("Signups",d.signupsCsv,sups=>{
   state.signups=sups.map(x=>({
    activityId:normalizeActivityId(x.ActivityID||x["Activity ID"]||x.activityId),
    status:String(x.Status||x.status||"Going").trim(),
    people:Number(x.People||x["Number of People"]||x.people||1)
   }));
   signupsReady=true;
  },renderActivities),
...
function formatTimeValue(v){
 ...
 // Google Sheets can export a time as a fraction of a day.
 if(/^\d*\.?\d+$/.test(raw)){
  const n=Number(raw);
  if(n>=0 && n<1){
 ...
 // If Sheets emits a full date/time string, keep only the time portion.
...
function countFor(id,status){
 const targetId=normalizeActivityId(id);
 const targetStatus=normalizeStatus(status);

 return state.signups
  .filter(s=>
   normalizeActivityId(s.activityId)===targetId &&
   normalizeStatus(s.status)===targetStatus
  )
...
console.info("[Trip Site] V2.6 started");
renderAll();
countdown();
loadData();
